...
def _has_tracked_changes(doc: Document) -> bool:
    ...
    element behind an ordinary PAGE/TOC/REF/hyperlink field — and
    ``<w:insideH>``/``<w:insideV>`` — the inside-border sides of a table's
    ``<w:tblBorders>``/``<w:tcBorders>``. Both are far more common in an
    everyday document than an actual tracked change, so a document with a
    page-number field or a bordered table was reported as having tracked
    changes it does not have. SKILL.md promises ``has_tracked_changes: true``
    only "when any w:ins or w:del element is found", so this walks the
    element tree for the exact tag instead of a string that merely prefixes
    it.
    """
